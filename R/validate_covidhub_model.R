#' Validate a COVID-19 Forecast Hub submission file
#'
#' Performs a full validation of a forecast submission file against the
#' COVID-19 Forecast Hub hubverse-format specification. Checks file
#' structure, column types, allowed values, cross-column logical
#' consistency, and group-level completeness rules. Errors are collected
...
#' file format, not modeling decisions. Cross-location joint-sample
#' structure (same `output_type_id` shared across locations) is permitted
#' but not deeply validated in this version.
#'
#' @param file Path to a COVID-19 Forecast Hub submission CSV file.
...
#'
#' @export
validate_covidhub_model <- function(file, verbose = FALSE) {
...
#------------------------------------------------------------------------------#
  ...
  abort_if_errors <- function(){
    ...
    if(length(errors) > 0){
      ...
      stop(
        "COVIDHub model validation failed with ", length(errors), " error(s):\n  - ",
...
#------------------------------------------------------------------------------#
# Defining COVID-19 Forecast Hub lookup tables ---------------------------------
#------------------------------------------------------------------------------#
# About: This section centralizes all COVID-19 Forecast Hub specification      #
# constants. When the spec changes, update the values here and the rest of     #
...
#------------------------------------------------------------------------------#
  ...
  ##############################
  # Note: COVIDHub spec uses "samples" (plural), distinct from FluSight's
  # "sample" (singular). Mirroring the spec exactly here.
  allowed_output_types <- c("quantile", "samples")

  #################################################
  # Map from target to allowed output_type values #
  #################################################
  target_output_type_map <- list(
    "wk inc covid hosp"           = c("quantile", "samples"),
    "wk inc covid prop ed visits" = "quantile"
  )
  ...
  ############################################################
  # Allowed horizon values (same range as FluSight non-peak) #
  ############################################################
  allowed_horizons <- -1:3

  #################################################
  # Allowed locations from the hubverse crosswalk #
  #################################################
...
# COVIDHub columns. Missing required columns or extra unexpected columns are   #
...
#------------------------------------------------------------------------------#
  ...
  if(length(bad_ref_dow) > 0){
    ...
    add_error(paste0(
      ...
      "COVIDHub reference_date must be the Saturday following the Submission ",
  ...
  # Checking for errors -- COVIDHub has no peak targets, so NA is never allowed
  ...
  if(length(bad_ted_dow) > 0){
    ...
    add_error(paste0(
      ...
      " row(s) that are not on a Saturday. ",
      "Each target_end_date must be the Saturday ending a US CDC epiweek. ",
  ...
  # Determining if any horizons 'errored' out -- COVIDHub never allows NA horizon
  ...
  ##############################################
  # location must be in the hubverse crosswalk #
  ##############################################
  ...
  if(length(bad_loc_unknown) > 0){
    ...
    add_error(paste0(
      ...
      " row(s) with values not in the hubverse location crosswalk. ",
...
#------------------------------------------------------------------------------#
  ...
  # Collect violating rows in a vector
  ...
  # Determining the `samples` rows (note: COVIDHub uses plural "samples")
  s_rows <- which(data$output_type == "samples")
...
# all 23 quantile levels per quantile group, monotone quantile values, at      #
# least one sample ID per sample group, and no duplicate horizons within a     #
# single sample ID. Cross-location joint-sample structure is permitted but     #
# not deeply validated in this version.                                        #
#------------------------------------------------------------------------------#
  ...
  #######################################################
  # Sample completeness: at least 1 ID per sample group #
  #######################################################

  # Pulling rows that are samples
  s_data <- data[data$output_type == "samples", ]

  # Running if samples are present
  if(nrow(s_data) > 0){

    # Creating unique group key per (reference_date, location, target)
    ...
    # Empty vector to store empty groups
    bad_count <- character()
    ...
    for(g in names(grp_split)){
      ...
      # Determining if no sample IDs at all
      if(n_unique < 1){bad_count <- c(bad_count, g)}
    ...
    if(length(bad_count) > 0){
      ...
      # Error message to show to users
      add_error(paste0(
        length(bad_count), " sample group(s) have no sample IDs. ",
        "First bad group: ", bad_count[1], "."
      ))
  ...
  ###################################################
  # No duplicate horizons within a single sample ID #
  ###################################################
  ...
  if(nrow(s_data) > 0){
    ...
    s_data$.grp <- paste(
      s_data$reference_date,
      s_data$location,
      s_data$target,
      sep = "|"
    )

    # Empty vector to collect bad groups
    bad_traj_dup <- character()

    # Need full row data, not just IDs, to access horizon
    s_data_full <- data[data$output_type == "samples", ]
    s_data_full$.grp <- paste(s_data_full$reference_date,
                              s_data_full$location,
                              s_data_full$target, sep = "|")

    # Split by group
    grp_split_traj <- split(s_data_full, s_data_full$.grp)
    ...
    for(g in names(grp_split_traj)){
      ...
      # Check no duplicates within an ID
      for(id in names(horizon_by_id)){

        # Indexed horizon vector
        h_vec <- horizon_by_id[[id]]

        # Duplicate horizons within an ID
        if(any(duplicated(h_vec))){
          bad_traj_dup <- c(bad_traj_dup, paste0(g, " / id=", id))
...
#------------------------------------------------------------------------------#
# About: This section enforces remaining COVIDHub specification rules: ED      #
# visit values constrained to [0, 1], and global uniqueness of the row key.    #
#------------------------------------------------------------------------------#
  ...
  # Pulling ED rows
  ed_rows <- which(data$target == "wk inc covid prop ed visits")
  ...
  if(length(bad_ed) > 0){
    ...
    add_error(paste0(
      ...
      " out-of-range value(s) for target 'wk inc covid prop ed visits' ",
...
#------------------------------------------------------------------------------#
  ...
  if(isTRUE(verbose)){
    ...
    # Message to print
    message(
      "\u2713 COVIDHub model validated successfully.\n",
      "  File:         ", file, "\n",
