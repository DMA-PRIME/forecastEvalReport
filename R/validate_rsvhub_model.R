#' Validate an RSV Forecast Hub submission file
#'
#' Performs a full validation of a forecast submission file against the
#' RSV Forecast Hub hubverse-format specification. Checks file structure,
...
#'
#' @param file Path to an RSV Forecast Hub submission CSV file.
...
#'
#' @export
validate_rsvhub_model <- function(file, verbose = FALSE) {
...
#------------------------------------------------------------------------------#
  ...
  abort_if_errors <- function(){
    ...
    if(length(errors) > 0){
      ...
      stop(
        "RSV Hub model validation failed with ", length(errors), " error(s):\n  - ",
...
#------------------------------------------------------------------------------#
# Defining RSV Forecast Hub lookup tables --------------------------------------
#------------------------------------------------------------------------------#
# About: This section centralizes all RSV Forecast Hub specification           #
...
#------------------------------------------------------------------------------#
  ...
  ##############################
  # Note: RSV Hub spec uses "samples" (plural), matching COVIDHub.
  # Mirroring the spec exactly here.
  ...
  target_output_type_map <- list(
    "wk inc rsv hosp"           = c("quantile", "samples"),
    "wk inc rsv prop ed visits" = "quantile"
  )
...
# RSV Hub columns. Missing required columns or extra unexpected columns are    #
...
#------------------------------------------------------------------------------#
  ...
  if(length(bad_ref_dow) > 0){
    ...
    add_error(paste0(
      ...
      "RSV Hub reference_date must be the Saturday following the Submission ",
  ...
  # Checking for errors -- RSV Hub has no peak targets, so NA is never allowed
  ...
  # Determining if any horizons 'errored' out -- RSV Hub never allows NA horizon
...
#------------------------------------------------------------------------------#
  ...
  # Determining the `samples` rows (note: RSV Hub uses plural "samples")
...
#------------------------------------------------------------------------------#
# About: This section enforces remaining RSV Hub specification rules: ED       #
...
#------------------------------------------------------------------------------#
  ...
  # Pulling ED rows
  ed_rows <- which(data$target == "wk inc rsv prop ed visits")
  ...
  if(length(bad_ed) > 0){
    ...
    add_error(paste0(
      ...
      " out-of-range value(s) for target 'wk inc rsv prop ed visits' ",
...
#------------------------------------------------------------------------------#
  ...
  if(isTRUE(verbose)){
    ...
    # Message to print
    message(
      "\u2713 RSV Hub model validated successfully.\n",
      "  File:         ", file, "\n",
