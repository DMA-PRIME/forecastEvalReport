#' Create a report options file template
#'
#' Writes a pre-filled R options file template to the current working
#' directory. The template contains all parameters required by
#' `generate_report()`, with inline comments explaining each field's
#' purpose and acceptable values. Users open the file, fill in their
#' values, and follow the workflow described in the template header to
#' produce their report.
#'
#' The output filename is `report_options_template.R`. If a file with
#' that name already exists in the working directory, the function stops
#' with an error unless `force = TRUE` is set.
#'
#' @param force Logical. If `TRUE`, overwrites an existing template file.
#'   Defaults to `FALSE`.
#'
#' @return Invisibly returns the path to the written template file.
#'
#' @export
create_options_template <- function(force = FALSE) {

#------------------------------------------------------------------------------#
# Checking for existing file ---------------------------------------------------
#------------------------------------------------------------------------------#
# About: This section checks if the template file already exists in the        #
# working directory. If it does, the function stops unless force = TRUE is     #
# set.                                                                         #
#------------------------------------------------------------------------------#
  ...
  ##################################
  output_path <- file.path(getwd(), "report_options_template.R")

  ########################################
  # Stopping if file exists without force #
  ########################################
  if(file.exists(output_path) && !isTRUE(force)){

    # Stopping script if the file exisits
    stop(
      "A template file already exists at:\n  ", output_path, "\n\n",
      "To overwrite it, re-run with force = TRUE:\n",
      "  create_options_template(force = TRUE)\n\n",
      "WARNING: Overwriting will permanently delete any edits you have made.",
      call. = FALSE
    )
...
#------------------------------------------------------------------------------#
# About: This section writes the pre-filled options file template. Each field  #
# is documented with inline comments explaining its purpose, acceptable        #
# values, and whether it is required or optional.                              #
#------------------------------------------------------------------------------#
...
#------------------------------------------------------------------------------#
# About: This section prints the message to the user with the path name of the #
# created file and the next steps to generate the report.                      #
#------------------------------------------------------------------------------#
  ...
  message(
    ...
    "\n",
    "  Next steps:\n",
    "    1. Open the file and fill in all REQUIRED fields.\n",
    "       Leave variables.crosswalk.file = NA for now.\n",
    "\n",
    "    2. Build the variables crosswalk:\n",
    "         build_crosswalk_from_options('", output_path, "')\n",
    "\n",
    "    3. Complete the crosswalk file (clean names, definitions, etc.).\n",
    "\n",
    "    4. Add the crosswalk file path to variables.crosswalk.file.\n",
    "\n",
